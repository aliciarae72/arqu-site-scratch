...
(function () {
  ...
  function lift(x, y, now) {
    var dx = x - P.x,
      ...
      f = 0,
      g = 0;
    if (P.von > 0.01 && d < REACH) {
      f = 1 - d / REACH;
      f = f * f * (3 - 2 * f) * P.von;
    }
    for (var i = 0; i < rings.length; i++) {
      var age = now - rings[i].t,
        rr = age * 0.42;
      var rx = x - rings[i].x,
        ry = y - rings[i].y,
        e = (Math.sqrt(rx * rx + ry * ry) - rr) / 28;
      g = Math.max(g, Math.exp(-e * e) * (1 - age / 1700));
    }
    return { f: f, g: g, dx: d ? dx / d : 0, dy: d ? dy / d : 0 };
  }

  function drawDot(x, y, now) {
    var L = lift(x, y, now),
      inCol = x > colL && x < colR;
    var push = L.f * 7,
      hot = Math.max(L.f, L.g);
    var a = (inCol ? 0.05 : 0.085) * (1 - hot);
    if (a > 0.004) {
      ctx.fillStyle = 'rgba(34,31,32,' + a.toFixed(3) + ')';
  ...
  /* keep drawing only while the halo is still easing or a ring is still travelling */
  function settling() {
    return (
      Math.abs(P.tx - P.x) > 0.3 || Math.abs(P.ty - P.y) > 0.3 || Math.abs(P.on - P.von) > 0.01 || rings.length > 0
    );
  }
  ...
  function frame(now) {
    ...
    var oy = -((window.scrollY * 0.18) % GAP),
      ox = (w % GAP) / 2;
    for (var y = oy; y < h + GAP; y += GAP) {
      for (var x = ox; x < w + GAP; x += GAP) drawDot(x, y, now);
    }
    if (settling()) kick();
  }
  function kick() {
