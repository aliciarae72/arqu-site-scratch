...
const MARKS = [
  { sel: '#hero-title', text: 'beyond', kind: 'circle' },
  { sel: '.val:nth-child(2) h3', text: 'Human touch', kind: 'underline' },
  { sel: '.way-grid .or', text: 'or', kind: 'circle' },
  { sel: '[data-rn] .rn-head h3', text: '93 seconds', kind: 'underline' },
...
test('an underline is one fluid stroke: a single path, drawn in a single pass', async (t) => {
  ...
  await scrollToSelector(page, '[data-rn] .rn-head', 200);
  await page.waitForFunction(() => document.querySelector('[data-rn] .rn-head > .hand-mark > g'), null, {
    timeout: 6000,
  });
  const paths = await page.evaluate(() =>
    [...document.querySelectorAll('[data-rn] .rn-head > .hand-mark path')].map((p) => p.getAttribute('d')),
  );
